...
# ---------------------------------------------------------------------------
# Session state – load weights per person; reload when person changes
# ---------------------------------------------------------------------------
if st.session_state.get("_last_person") != selected_person:
    st.session_state["_last_person"] = selected_person
    _person_exercises = build_person_exercises(selected_person)
    st.session_state.weights = load_weights(selected_person, _person_exercises)
    st.session_state.weights_saved = False
    for _k in [k for k in st.session_state if k.startswith(("sl_", "ni_"))]:
...
# Build this person's exercise dict for use throughout the page
_current_exercises = build_person_exercises(selected_person)
...
def _make_save_cb(muscle: str, movement: str, wtype: str, widget_key: str):
    ...
    def _cb():
        ...
        save_weights(st.session_state.weights, st.session_state.selected_person)
...
# ---------------------------------------------------------------------------
def _render_theme_tab(theme_movements: set[str], tab_title: str, search_key: str, exercises: dict) -> None:
    ...
    for body_part_label, muscles_in_part in BODY_PARTS:
        ...
        for muscle_name in muscles_in_part:
            if muscle_name not in exercises:
                continue
            movs = [mv for mv in exercises[muscle_name] if mv in theme_movements]
            if not movs:
                continue
            # Apply search filter
            if search:
        ...
        for muscle_name, movements in relevant:
            with st.expander(f"🦾  {muscle_name}", expanded=False):
                for i, movement in enumerate(movements):
                    entry = exercises[muscle_name][movement]
...
weekly_plan = build_weekly_plan(_current_exercises, st.session_state.weights)
...
# ===========================================================================
with tab_flexion:
    _render_theme_tab(FLEXION_MOVEMENTS, "💪  Beugung (Flexion)", "flexion", _current_exercises)
...
# ===========================================================================
with tab_extension:
    _render_theme_tab(EXTENSION_MOVEMENTS, "📐  Streckung (Extension)", "extension", _current_exercises)
...
# ===========================================================================
with tab_rotation:
    _render_theme_tab(ROTATION_MOVEMENTS, "🔄  Rotation & mehr", "rotation", _current_exercises)
